...
class MonoImagePublisher : public rclcpp::Node
{
    public:
        ...
        {
            std::cout << "节点已启动：mono_image_publisher"<<std::endl;
            ...
            image_pub_ = this->create_publisher<sensor_msgs::msg::Image>(namespace_ + "/camera_ori_data", 2);
            ...
            std::string cmd1 = " ! video/x-raw(memory:NVMM), width=640, height=480, format=(string)NV12, framerate=(fraction)";
    ...
    private:
        ...
        { 
            Mat frame;
            cam_.read(frame);
	    if (frame.empty()) throw std::runtime_error("cannot read frame0 from camera");
